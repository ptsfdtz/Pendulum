...
% MATLAB vendor I/O only; the replaceable algorithm is in Control.
...
block.NumInputPorts=1; block.NumOutputPorts=4;
block.SetPreCompInpPortInfoToDynamic; block.SetPreCompOutPortInfoToDynamic;
block.InputPort(1).SamplingMode='Sample';
for k=1:4, block.OutputPort(k).SamplingMode='Sample'; end
...
b.OutputPort(4).Data=double(ctx.Armed && ~ctx.Finished);
end
function write(b)
