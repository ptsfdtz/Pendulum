...
void NI6602::configureSecondPendulumEncoderRaw(
    ...
    if (counter.empty() || counter == "UNCONFIRMED" || phaseATerminal.empty() ||
        ...
        phaseBTerminal == "UNCONFIRMED" ||
        !std::isfinite(filterMinPulseWidthSeconds) || filterMinPulseWidthSeconds < 0.0) {
        throw std::invalid_argument("Invalid or unconfirmed second pendulum encoder configuration");
    }
    const bool useDefaultRouting = phaseATerminal == "DEFAULT" && phaseBTerminal == "DEFAULT";
    const bool useExplicitRouting = phaseATerminal != "DEFAULT" &&
                                    phaseBTerminal != "DEFAULT" &&
    ...
    try {
        ...
        if (!useDefaultRouting) {
            checkDaq(DAQmxSetCIEncoderAInputTerm(
                         impl_->secondPendulumEncoderTask.get(), "", phaseATerminal.c_str()),
                     "DAQmxSetCIEncoderAInputTerm(second pendulum encoder)");
            checkDaq(DAQmxSetCIEncoderBInputTerm(
                         impl_->secondPendulumEncoderTask.get(), "", phaseBTerminal.c_str()),
...
std::uint32_t NI6602::readSecondPendulumEncoderRaw() {
    if (!impl_->secondPendulumEncoderTask.valid()) {
        throw std::logic_error("Second pendulum encoder task has not been configured");
    }
    uInt32 value = 0;
    checkDaq(DAQmxReadCounterScalarU32(impl_->secondPendulumEncoderTask.get(), 1.0,
                                      &value, nullptr),
